...
import type { CaseSpec, HealerFn, ReplayFile, ReplayStep, RunOptions, RunResult, StepResult } from '../types.js';
...
export async function replayCase(replay: ReplayFile, options: RunOptions, healer?: HealerFn): Promise<RunResult> {
  ...
  try {
    ...
    for (let i = 0; i < replay.steps.length; i++) {
      ...
      try {
        ...
        stepResults.push({ index: i, step, status: 'passed', durationMs: Date.now() - t0 });
        continue;
      } catch (err) {
        ...
        if (healer) {
          ...
          if (fixedStep) {
            try {
              ...
              healedAny = true;
              stepResults.push({ index: i, step: fixedStep, status: 'healed', error, durationMs: Date.now() - t0 });
              continue;
            } catch (retryErr) {
              stepResults.push({
                index: i,
        ...
        stepResults.push({ index: i, step, status: 'failed', error, durationMs: Date.now() - t0 });
        verdict = 'failed';
  ...
  return {
    ...
    explanation,
    steps: stepResults,
    artifacts: { videoPath, replayPath: healedAny ? replayPath : undefined, screenshots: [] },
    startedAt,
